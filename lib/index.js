 ...
 * 所以这里全部拍平成一级字段。
 *
 * `z` 由调用方注入(动态 import `@deepseek-ai/schemastery`):那是 harness 自带的
 * 官方包,按 awesome-dsh-plugin 注册表规范应声明为 peerDependency 而不是
 * dependency。所以这里不做静态导入——拿不到就跳过设置命名空间、只用
 * cordis.patch.yml 的配置,插件其余功能完全不受影响。
 */
function buildSettingsSchema(z) {
  return z.object({
    enabled: z.boolean().default(DEFAULT_CONFIG.enabled),
    holdThresholdMs: z.number().min(150).max(2000).default(DEFAULT_CONFIG.holdThresholdMs),
    cancelSlidePx: z.number().min(20).max(300).default(DEFAULT_CONFIG.cancelSlidePx),
    interimIntervalMs: z.number().min(400).max(5000).default(DEFAULT_CONFIG.interimIntervalMs),
    minHoldMs: z.number().min(0).max(3000).default(DEFAULT_CONFIG.minHoldMs),
    maxHoldMs: z.number().min(5000).max(180000).default(DEFAULT_CONFIG.maxHoldMs),
    maxWindowSec: z.number().min(4).max(30).default(DEFAULT_CONFIG.maxWindowSec),
    autoSend: z.boolean().default(DEFAULT_CONFIG.autoSend),
    language: z.string().default(DEFAULT_CONFIG.language),
    useItn: z.boolean().default(DEFAULT_CONFIG.useItn),
    numThreads: z.number().min(1).max(8).default(DEFAULT_CONFIG.numThreads),
    mirror: z.string().default(DEFAULT_CONFIG.mirror),
    modelDir: z.string().default(DEFAULT_CONFIG.modelDir),
  });
}
...
export function apply(ctx) {
  ...
  // webServer 服务同样可能晚于本插件就绪(与 settings 同理),用 ctx.inject 等它
  ...
  // 设置命名空间:Web 设置面板的数据层。settings 服务由 dsh-settings-file 提供,
  // apply 期未必就绪,直接 ctx.get("settings") 会拿到 undefined 并静默跳过注册。
  ctx.inject(["settings"], (settingsCtx) => {
    // 动态导入官方 schema 库:它是 peerDependency(注册表规范),缺席时要优雅降级,
    // 不能让整个插件加载失败。用 IIFE 是为了让 inject 回调本身保持同步。
    void (async () => {
      let z;
      try {
        ({ default: z } = await import("@deepseek-ai/schemastery"));
      } catch (err) {
        log("info", `${NS}: 未找到 @deepseek-ai/schemastery,跳过设置命名空间(改用 cordis.patch.yml 配置)`);
        return;
      }
      try {
        const scope = settingsCtx.settings.register(NS, buildSettingsSchema(z), { base: { ...config } });
        const resolved = scope.get();
        if (resolved) applyConfig(resolved);
        scope.watch((next) => {
          if (next) applyConfig(next);
        });
      } catch (err) {
        log("warn", `${NS}: 设置命名空间注册失败,使用 cordis.patch.yml 默认值:${String((err && err.message) || err)}`);
      }
    })();
  });

  function applyConfig(next) {
    const engineKeys = ["language", "useItn", "numThreads", "modelDir", "mirror"];
    const before = config;
    config = { ...DEFAULT_CONFIG, ...next };
    state.model.dir = modelsDirSafe();
    const engineChanged = engineKeys.some((key) => before[key] !== config[key]);
    if (engineChanged && worker) {
      log("info", `${NS}: 识别引擎配置变化,重启 worker`);
      shutdownEngine();
    }
  }
